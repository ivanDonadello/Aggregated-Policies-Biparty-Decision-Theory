...
class Simulations:
    ...
    def to_csv(self, filename):
        with open(filename, mode='w') as file:
            ...
            for node_id, node in self.dict_tree.items():
                writer.writerow([node_id, node.text, [child.id for child in node.children], node.utility_proponent,
                                 node.utility_opponent])
    ...
    def results_to_df(self):
        header = ["profile_id", "Node id", "Proponent utility", "Opponent utility", "Q_proponent_root",
                  "Q_opponent_root"]
    ...
    def set_opponent_utilities(self, sample, predicted=False):
        for id_arg in self.extra_data["frontier"]:
            self.dict_tree[id_arg].utility_opponent = int(sample[id_arg]) if type(sample[id_arg]) is str else sample[
                id_arg]

            if id_arg in self.extra_data["same_as"]:
                for same_arg in self.extra_data["same_as"][id_arg]:
                    self.dict_tree[same_arg].utility_opponent = int(sample[id_arg]) if type(sample[id_arg]) is str else \
                    sample[id_arg]